    ...
    public void parseNews(String newsTheme) { //RIA
        try {
            ...
            newsElements.forEach(element -> {
                ...
                Element contentRoot = element.getElementsByClass("list-item__content").subList(0, 1).get(0); //тег для контента
                List<Element> contentsLinkTags = contentRoot.getElementsByTag("a").subList(0, 2);
                String newsLink = contentsLinkTags.get(0).attr("href"); //ссылка на ресурс
                String newsName = contentsLinkTags.get(1).text(); // название
                ...
                if (newsRecordRepository.findByContentLink(newsRecord.getContentLink()) == null && pictures.size() != 0) {
